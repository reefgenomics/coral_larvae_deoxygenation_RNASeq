...
cnt=read.table("./Input_files/FinalTable_counts_ASL.txt", header=TRUE, row.names = 1, sep= "\t") #eXpress results
cnts=round(cnt, 0)
cnts.sub=subset(cnts, rowSums(cnts) > 50)
cnts.sor=cnts.sub[ , order(names(cnts.sub))]
colnames(cnts.sor)=gsub("\\.", "-", colnames(cnts.sor))

map.a=subset(map, map$Time =="T2") #change accordingly for T1-3
cnt.a=cnts.sor[,(names(cnts.sor)[(names(cnts.sor) %in% row.names(map.a))])] #make names to match up in same order

dds.a=DESeqDataSetFromMatrix(countData = as.matrix(cnt.a), colData = map.a, design = ~Group)
dds.a=estimateSizeFactors(dds.a)
dds.a=estimateDispersions(dds.a)
vsd.a=vst(dds.a)

COL2=c("#99CCFF", "#333399")
pca.vsd=plotPCA(vsd.a, intgroup=c("Condition", "Time"), ntop = 1000, returnData = TRUE) 
#intgroup=interesting groups is a character vector of names in colData(x) to use for grouping
